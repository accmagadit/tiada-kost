            ...
            <div class="col-xl-12 mb-5 mb-xl-0">
                <div class="card shadow">
                    <div class="card-header border-0">
                        ...
                        <br><br>
                        <form method="post" action="/uangbulanan-backend">
                            @csrf
                            <div class="form-group">
                                ...
                                <select class="form-control" name="penghuni_id" id="penghuni_id">
                                    @foreach ($penghunis as $penghuni)
                                        <option value="{{ $penghuni->id }}">{{ $penghuni->nama_penghuni}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="id_tipe_pembayaran" id="tipe_pembayaran_id">
                                    @foreach ($tipePembayarans as $tipePembayaran)
                                        <option value="{{ $tipePembayaran->id }}">{{ $tipePembayaran->jumlah_bulan }}</option>
                                    @endforeach
                                </select>
                            </div>
